...
aai.settings.api_key = "your assemby ai key"  # Replace with your actual API key
...
def generate_blog_from_transcription(transcription,getlanguage):
    try:
        model = genai.GenerativeModel("gemini-1.5-pro-latest")
        response = model.generate_content(f"Write a blog based on this transcription in {getlanguage} and in proper structured way: {transcription}")
...
def generate_blog(request):
    ...
    if request.method == "POST":
        try:
            ...
            # Generate blog
            blog_content = generate_blog_from_transcription(transcription,getlanguage)
